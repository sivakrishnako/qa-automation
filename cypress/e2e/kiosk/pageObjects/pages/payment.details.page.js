...
{  
    ...
    static checkboxForNewCreditDebitCard=('[data-testid="debit-credit-card"] > .MuiRadio-root > .PrivateSwitchBase-input')
    static checkBoxForNewElectronicCheck=('.MuiGrid-container > .MuiGrid-root > :nth-child(1) > [data-testid="electronic-check"] > .MuiRadio-root > .PrivateSwitchBase-input')
...
static savedCardImage(){
    cy.get('[data-testid="save-card"]',{ timeout: Cypress.env('elementTimeout')})
}
static nameOnSavedCard=('[data-testid="name-of-card"]')
...
static clickOnMakePayment(){
    const button=cy.get('[data-testid="make-payment"]').invoke('removeAttr', 'target')
button.click({ force: true })
    return this;
}
...
static clickOnDropDownOfExitKiosk(){
    const button=cy.get('[data-testid="KeyboardArrowDownIcon"]',{ timeout: 10000 })
    button.click({ force: true })
    return this
}
...
static getPopUpForPayAtFrontDesk(){
